...
CREATE TABLE IF NOT EXISTS `account_balance`.`account` (
  ...
  CONSTRAINT `fk_bank_account_bank`
    ...
    REFERENCES `account_balance`.`provider` (`provider_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_bank_account_account_type1`
    FOREIGN KEY (`account_type_id`)
    REFERENCES `account_balance`.`account_type` (`account_type_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `account_balance`.`transaction` (
  ...
  CONSTRAINT `fk_transaction_transaction_type1`
    ...
    REFERENCES `account_balance`.`transaction_type` (`transaction_type_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_transaction_bank_account1`
    FOREIGN KEY (`account_id`)
    REFERENCES `account_balance`.`account` (`account_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `account_balance`.`scheduled_transaction` (
  `scheduled_transaction_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `account_id` INT UNSIGNED NOT NULL,
  `transaction_type_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `account_balance`.`transaction_detail` (
  ...
  CONSTRAINT `fk_transaction_detail_transaction1`
    ...
    REFERENCES `account_balance`.`transaction` (`transaction_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `account_balance`.`user_account` (
  ...
  CONSTRAINT `fk_user_account_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `account_balance`.`user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
